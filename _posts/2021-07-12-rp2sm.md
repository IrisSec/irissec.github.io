...
struct RtsMethodInfo {
	int ptr;
	int size;
	//unsure about these two, but they should be 02 and 01 respectively
	byte argCount;
	byte returnCount;
	byte localCount;
	byte unk;
...
0x13: //jump to label unconditionally by index
	jump to label[arg0(8)]
	(also an extra arg1(8), idk what it does)
0x14: //jump to label if stack is 0
	pop rax
	test eax, eax
	jne label[arg0(8)]
	(also an extra arg1(8))
0x20: //load local
...
Sadly, our instruction set doesn't have a lot of the things we need to make this work. There's no multiply or divide instructions, only add and subtract, and we also can only jump to labels that have already been placed (as far as I know) so you can't really do if statements.

Here's what I did (I probably overcomplicated something somewhere, but it works for me)
...
Since we can't "break" out of a loop (since we can't jump to a label), all loop code must happen in one place, we can't have a break inbetween. For example, this would not work:
...
The if statement needs a forwards jump, which I can't do (that I know of). So I just left out the code for the if statement entirely. It's random if this ever gets called, so we can just keep running the script until it spits out a valid result.

Edit: you can jump forwards. I'm an idiot.
...
I did all of the assembly in text first, then manually typed the hex later.
